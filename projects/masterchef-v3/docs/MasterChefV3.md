...
### latestPeriodQstPerSecond

```solidity
uint256 latestPeriodQstPerSecond
```
...
```solidity
uint256 qstAmountBelongToMC
```

Record the qst amount belong to MasterChefV3.
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, contract IQubeV3Pool v3Pool, contract ILMPool lmPool)
```
...
```solidity
event NewUpkeepPeriod(uint256 periodNumber, uint256 startTime, uint256 endTime, uint256 qstPerSecond, uint256 qstAmount)
```

### UpdateUpkeepPeriod

```solidity
event UpdateUpkeepPeriod(uint256 periodNumber, uint256 oldEndTime, uint256 newEndTime, uint256 remainingQst)
```
...
Returns the qst per second , period end time.
...
Returns the qst per second , period end time. This is for liquidity mining pool.
...
```solidity
function add(uint256 _allocPoint, contract IQubeV3Pool _v3Pool, bool _withUpdate) external
```
...
| _v3Pool | contract IQubeV3Pool | Address of the V3 pool. |
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _amount | uint256 | The amount of qst injected. |
...
Update qst reward for all the liquidity mining pool.
...
Update qst reward for the liquidity mining pool.
