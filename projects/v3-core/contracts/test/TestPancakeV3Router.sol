...
contract TestQubeV3Router is IQubeV3SwapCallback {
    ...
    function qubeV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IQubeV3Pool(msg.sender).token0()
                : IQubeV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IQubeV3Pool(pools[0]).token1();
            IQubeV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IQubeV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IQubeV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
