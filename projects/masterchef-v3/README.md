...
Record the qst amount belong to 
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| qstPerSecond | uint256 | Qst reward per second. |
...
```solidity
function add(uint256 _allocPoint, IQubeV3Pool _v3Pool, bool _withUpdate) external
```
...
| _v3Pool | IQubeV3Pool | Address of the V3 pool. |
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, IQubeV3Pool v3Pool, ILMPool lmPool)
```
