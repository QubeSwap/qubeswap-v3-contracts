...
import '@qubeswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@qubeswap/v3-periphery/contracts/libraries/Path.sol';
import '@qubeswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@qubeswap/v3-core/contracts/interfaces/IQubeV3Pool.sol';
import '@qubeswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IQubeV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IQubeV3Pool pool) {
        pool = IQubeV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IQubeV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IQubeV3Pool(pool).slot0();
            }
