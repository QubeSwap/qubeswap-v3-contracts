...
The contract that deployed the pool, which must adhere to the IQubeV3Factory interface
...
```solidity
struct QubeV3Pool.ProtocolFees protocolFees
```
...
_Prevents calling a function from anyone except the address returned by IQubeV3Factory#owner()_
...
_The caller of this method receives a callback in the form of IQubeV3SwapCallback#qubeV3SwapCallback_
...
_The caller of this method receives a callback in the form of IQubeV3FlashCallback#qubeV3FlashCallback
...
```solidity
function setLmPool(contract IQubeV3LmPool _lmPool) external
```
