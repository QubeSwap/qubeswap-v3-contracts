...
async function main() {
  ...
  const deployedContracts_v3_core = await import(`@qubeswap/v3-core/deployments/${networkName}.json`)
  const deployedContracts_v3_periphery = await import(`@qubeswap/v3-periphery/deployments/${networkName}.json`)

  // Verify swapRouter
  console.log('Verify swapRouter')
  await verifyContract(deployedContracts_v3_periphery.SwapRouter, [
    deployedContracts_v3_core.QubeV3PoolDeployer,
    deployedContracts_v3_core.QubeV3Factory,
    config.WNATIVE,
  ])
  await sleep(10000)
  ...
  // Verify qubeInterfaceMulticall
  console.log('Verify qubeInterfaceMulticall')
  await verifyContract(deployedContracts_v3_periphery.QubeInterfaceMulticall)
  await sleep(10000)
