...
import QubeV3PoolDeployerArtifact from "@qubeswap/v3-core/artifacts/contracts/QubeV3PoolDeployer.sol/QubeV3PoolDeployer.json";
import QubeV3FactoryArtifact from "@qubeswap/v3-core/artifacts/contracts/QubeV3Factory.sol/QubeV3Factory.json";
// import QubeV3FactoryOwnerArtifact from "@qubeswap/v3-core/artifacts/contracts/QubeV3FactoryOwner.sol/QubeV3FactoryOwner.json";
import QubeV3SwapRouterArtifact from "@qubeswap/v3-periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json";
import NftDescriptorOffchainArtifact from "@qubeswap/v3-periphery/artifacts/contracts/NonfungibleTokenPositionDescriptorOffChain.sol/NonfungibleTokenPositionDescriptorOffChain.json";
import NonfungiblePositionManagerArtifact from "@qubeswap/v3-periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json";
import QubeV3LmPoolDeployerArtifact from "@qubeswap/v3-lm-pool/artifacts/contracts/QubeV3LmPoolDeployer.sol/QubeV3LmPoolDeployer.json";
...
describe("MasterChefV3", function () {
  ...
  beforeEach(async function () {
    ...
    // );
    // const baseTokenUri = "https://nft.qubeswap.com/v3/";
    // const nonfungibleTokenPositionDescriptor = await upgrades.deployProxy(NonfungibleTokenPositionDescriptor, [
    //   baseTokenUri,
    // ]);
    // await nonfungibleTokenPositionDescriptor.deployed();
    // TODO:
    await QubeV3SwapRouter.deploy(qubeV3PoolDeployer.address, qubeV3Factory.address, WETH9Address);
    ...
    // const QubeV3FactoryOwner = await ethers.getContractFactoryFromArtifact(QubeV3FactoryOwnerArtifact);
    // const qubeV3FactoryOwner = await QubeV3FactoryOwner.deploy(qubeV3Factory.address);
    ...
    const qubeV3LmPoolDeployer = await QubeV3LmPoolDeployer.deploy(
      masterChefV3.address
      // qubeV3FactoryOwner.address
    );
    // await qubeV3FactoryOwner.setLmPoolDeployer(qubeV3LmPoolDeployer.address);
    ...
    await tokenC.connect(admin).approve(qubeV3SwapRouter.address, ethers.constants.MaxUint256);
    await tokenD.connect(admin).approve(qubeV3SwapRouter.address, ethers.constants.MaxUint256);
  ...
  describe("Real world user flow", function () {
    context("when there are 2 users and 2 pools with no trading", function () {
      it("should executed successfully", async function () {
        ...
        console.log("@7 ----------------------------------------");
        console.log(`user1: ${ethers.utils.formatUnits(qstUser1)}`);
        console.log(`user2: ${ethers.utils.formatUnits(qstUser2)}`);
        console.log("");
        ...
        qstUser2 = (await this.qstToken.balanceOf(user2.address))
          ...
          .add(await this.masterChefV3.pendingQst(4))
          .add(await this.masterChefV3.pendingQst(6))
          .add(await this.masterChefV3.pendingQst(9));
        ...
        qstUser2 = (await this.qstToken.balanceOf(user2.address))
          ...
          .add(await this.masterChefV3.pendingQst(6))
          .add(await this.masterChefV3.pendingQst(9))
          .add(await this.masterChefV3.pendingQst(11));
        ...
        assert(qstUser1.sub(ethers.utils.parseUnits("57.68974359")).abs().lte(ethers.utils.parseUnits("0.1")));
        assert(qstUser2.sub(ethers.utils.parseUnits("105.3102564")).abs().lte(ethers.utils.parseUnits("0.1")));
      });
    });
    ...
    context("when there are 2 users and 1 pool with different range positions", function () {
      it("should executed successfully", async function () {
        ...
        assert(qstUser1.sub(ethers.utils.parseUnits("28.73260345")).abs().lte(ethers.utils.parseUnits("0.0000001")));
        assert(qstUser2.sub(ethers.utils.parseUnits("167.2673966")).abs().lte(ethers.utils.parseUnits("0.0000001")));
      });
    });
    ...
    context("when there are 2 users and 1 pool with different range positions", function () {
      it("should executed successfully", async function () {
        ...
        assert(qstUser1.sub(ethers.utils.parseUnits("73.99948783")).abs().lte(ethers.utils.parseUnits("0.0000001")));
        assert(qstUser2.sub(ethers.utils.parseUnits("122.0005122")).abs().lte(ethers.utils.parseUnits("0.0000001")));
      });
    });
  });
});
