 ...
 * Receiver will receive qst token, then upkeep for MasterChefV3.
 */
contract MasterChefV3KeeperV2 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    constructor(IMasterChefV3 _V3, IReceiver _receiver, IERC20 _qst) {
        MasterChefV3 = _V3;
        Receiver = _receiver;
        Qst = _qst;
    }
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            ...
            if (qstBalanceInReceiver > 0 && latestPeriodEndTime < block.timestamp + bufferSecond) upkeepNeeded = true;
        }
    }
