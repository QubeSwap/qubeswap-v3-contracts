...
```solidity
function getLatestPeriodInfo(address _v3Pool) external view returns (uint256 qstPerSecond, uint256 endTime)
```
...
```solidity
contract IQubeV3FactoryOwner factoryOwner
```
...
```solidity
constructor(address _masterChef, contract IQubeV3FactoryOwner _factoryOwner) public
```

### deploy

```solidity
function deploy(contract IQubeV3Pool pool) external returns (contract IQubeV3LmPool lmPool)
```
...
| ---- | ---- | ----------- |
| pool | contract IQubeV3Pool | The contract address of the QubeSwap V3 pool |
