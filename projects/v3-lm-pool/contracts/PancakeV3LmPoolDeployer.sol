...
import '@qubeswap/v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol';

import './QubeV3LmPool.sol';
...
contract QubeV3LmPoolDeployer {
    ...
    /// @param pool The contract address of the QubeSwap V3 pool
    function deploy(address pool) external onlyMasterChef returns (address lmPool) {
        ...
        lmPool = address(new QubeV3LmPool{salt: keccak256(abi.encode(pool, masterChef, block.timestamp))}());

        delete parameters;

        // Set new LMPool for qube v3 pool.
        IQubeV3Factory(INonfungiblePositionManager(IMasterChefV3(masterChef).nonfungiblePositionManager()).factory())
