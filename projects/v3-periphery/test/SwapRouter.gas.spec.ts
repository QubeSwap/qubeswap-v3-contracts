import { abi as IQubeV3PoolABI } from '@qubeswap/v3-core/artifacts/contracts/interfaces/IQubeV3Pool.sol/IQubeV3Pool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { IQubeV3Pool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IQubeV3Pool, IQubeV3Pool, IQubeV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IQubeV3PoolABI, wallet)) as [
      IQubeV3Pool,
      IQubeV3Pool,
      IQubeV3Pool
    ]
  ...
  let pools: [IQubeV3Pool, IQubeV3Pool, IQubeV3Pool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestQubeV3Callee')
