...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    uint256 public latestPeriodQstPerSecond;
    ...
    /// @notice Record the qst amount belong to MasterChefV3.
    uint256 public qstAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IQubeV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    /// @notice Returns the qst per second , period end time.
    /// @param _pid The pool pid.
    /// @return qstPerSecond Qst reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 qstPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            qstPerSecond = (latestPeriodQstPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the qst per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return qstPerSecond Qst reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 qstPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            qstPerSecond =
                (latestPeriodQstPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }
    ...
    function add(uint256 _allocPoint, IQubeV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotQubeNFT();
    ...
    /// @notice harvest qst from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Qst reward.
        ...
        if (_token == address(CAKE)) {
            unchecked {
                ...
                if (balance >= qstAmountBelongToMC) {
                    balance -= qstAmountBelongToMC;
                } else {
                    // This should never happend.
                    qstAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
        ...
        if (token == address(CAKE)) {
            unchecked {
                ...
                if (balanceToken >= qstAmountBelongToMC) {
                    balanceToken -= qstAmountBelongToMC;
                } else {
                    // This should never happend.
                    qstAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of qst injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        ...
        uint256 qstPerSecond;
        uint256 qstAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingQst = ((latestPeriodEndTime - currentTime) * latestPeriodQstPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingQst);
            qstAmount += remainingQst;
        }
        qstPerSecond = (qstAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodQstPerSecond = qstPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, qstPerSecond, qstAmount);
    }

    /// @notice Update qst reward for all the liquidity mining pool.
    ...
    /// @notice Update qst reward for the liquidity mining pool.
    ...
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            // Update qstAmountBelongToMC
            unchecked {
                if (qstAmountBelongToMC >= _amount) {
                    qstAmountBelongToMC -= _amount;
                } else {
