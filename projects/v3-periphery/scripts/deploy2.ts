...
const artifacts: { [name: string]: ContractJson } = {
  ...
  QubeInterfaceMulticall: require('../artifacts/contracts/lens/QubeInterfaceMulticall.sol/QubeInterfaceMulticall.json'),
...
async function main() {
  ...
  const qubeV3PoolDeployer_address = deployedContracts.QubeV3PoolDeployer
  const qubeV3Factory_address = deployedContracts.QubeV3Factory

  const SwapRouter = new ContractFactory(artifacts.SwapRouter.abi, artifacts.SwapRouter.bytecode, owner)
  const swapRouter = await SwapRouter.deploy(qubeV3PoolDeployer_address, qubeV3Factory_address, config.WNATIVE)

  // await tryVerify(swapRouter, [qubeV3PoolDeployer_address, qubeV3Factory_address, config.WNATIVE])
  ...
  const QubeInterfaceMulticall = new ContractFactory(
    artifacts.QubeInterfaceMulticall.abi,
    artifacts.QubeInterfaceMulticall.bytecode,
    owner
  )

  const qubeInterfaceMulticall = await QubeInterfaceMulticall.deploy()
  console.log('QubeInterfaceMulticall', qubeInterfaceMulticall.address)

  // await tryVerify(qubeInterfaceMulticall)
  ...
  const quoterV2 = await QuoterV2.deploy(qubeV3PoolDeployer_address, qubeV3Factory_address, config.WNATIVE)
  console.log('QuoterV2', quoterV2.address)

  // await tryVerify(quoterV2, [qubeV3PoolDeployer_address, qubeV3Factory_address, config.WNATIVE])
  ...
  const contracts = {
    ...
    QubeInterfaceMulticall: qubeInterfaceMulticall.address,
  }
