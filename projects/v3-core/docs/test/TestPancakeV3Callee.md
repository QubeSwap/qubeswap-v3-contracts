...
```solidity
function qubeV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes data) external
```

Called to `msg.sender` after executing a swap via IQubeV3Pool#swap.
...
| data | bytes | Any data passed through by the caller via the IQubeV3PoolActions#swap call |
...
```solidity
function qubeV3FlashCallback(uint256 fee0, uint256 fee1, bytes data) external
```

Called to `msg.sender` after transferring to the recipient from IQubeV3Pool#flash.
...
| data | bytes | Any data passed through by the caller via the IQubeV3PoolActions#flash call |
