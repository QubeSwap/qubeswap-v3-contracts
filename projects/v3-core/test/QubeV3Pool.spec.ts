...
import { TestQubeV3SwapPay } from '../typechain-types/contracts/test/TestQubeV3SwapPay'
...
import { TestQubeV3ReentrantCallee } from '../typechain-types/contracts/test/TestQubeV3ReentrantCallee'
...
describe('QubeV3Pool', () => {
  ...
  let factory: QubeV3Factory
  let pool: MockTimeQubeV3Pool

  let swapTarget: TestQubeV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeQubeV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestQubeV3ReentrantCallee')
      ).deploy()) as TestQubeV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestQubeV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestQubeV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestQubeV3SwapPay
