...
describe('QubeV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: QubeV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as QubeV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as QubeV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [QubeV3Factory, QubeV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('QubeV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('QubeV3Pool')
