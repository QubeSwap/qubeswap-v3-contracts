...
import '@qubeswap/v3-core/contracts/interfaces/callback/IQubeV3FlashCallback.sol';
...
/// @notice An example contract using the QubeSwap V3 flash function
contract PairFlash is IQubeV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `QubeV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IQubeV3Pool pool = IQubeV3Pool(PoolAddress.computeAddress(deployer, poolKey));
