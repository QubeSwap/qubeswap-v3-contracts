...
contract MasterChefV3Receiver is Ownable {
    ...
    /// @param _qst Qst token address.
    /// @param _v2Pid The pool id of the dummy pool on the MCV2.
    constructor(IMasterChefV2 _v2, IMasterChefV3 _v3, IERC20 _qst, uint256 _v2Pid) {
        MasterChefV2 = _v2;
        MasterChefV3 = _v3;
        Qst = _qst;
        V2_Pid = _v2Pid;

        Qst.safeApprove(address(_v3), type(uint256).max);
    }
    ...
    /// @notice Harvest pending QST tokens from MasterChef V2.
    ...
    /// @notice Withdraw unexpected tokens sent to the receiver, can also withdraw qst.
    /// @dev Callable by owner.
