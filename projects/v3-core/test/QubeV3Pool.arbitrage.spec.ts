...
import { QubeV3PoolSwapTest } from '../typechain-types/contracts/test/QubeV3PoolSwapTest'
...
describe('QubeV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('QubeV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as QubeV3PoolSwapTest
